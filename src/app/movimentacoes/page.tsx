...
export default function Movimentacoes() {
  ...
  const atualizarEstoqueProduto = (produtoId: number, novoEstoque: number) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await withLoading('salvando', async () => {
      ...
      // Salvar movimentação
      salvarMovimentacoes([novaMovimentacao, ...movimentacoes])

      // Atualizar estoque
      atualizarEstoqueProduto(produto.id, novoEstoque)
  ...
  const excluirMovimentacao = async (id: number) => {
    if (confirm('Tem certeza que deseja excluir esta movimentação?')) {
      await withLoading('excluindo', async () => {
        ...
        if (movimentacao) {
          // Reverter o estoque
          const produto = produtos.find(p => p.codigo === movimentacao.codigo)
          if (produto) {
            ...
            if (estoqueRevertido >= 0) {
              atualizarEstoqueProduto(produto.id, estoqueRevertido)
            }
          }
        }
  ...
  // Obter datas únicas
  const datasUnicas = [...new Set(movimentacoes.map(m => m.data))].sort().reverse()
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-7xl mx-auto py-4 sm:py-6 px-4 sm:px-6 lg:px-8">
        ...
        {isLoading('carregando') && (
          <div className="bg-white rounded-lg shadow-lg p-8 sm:p-12 mb-6">
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-gray-500 text-sm mt-2">Sincronizando dados do estoque</p>
            </div>
          </div>
        )}
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
                ...
                {formData.produtoId && formData.quantidade && (
                  <div className="bg-gradient-to-r from-blue-50 to-green-50 p-3 sm:p-4 rounded-lg border-2 border-blue-200">
                    <h4 className="font-bold text-gray-800 mb-2 text-sm sm:text-base">💰 Resumo da Movimentação:</h4>
                    {(() => {
                      const produto = produtos.find(p => p.id === parseInt(formData.produtoId))
                      const quantidade = parseInt(formData.quantidade)
                      ...
                      return (
                        <div className="space-y-1 text-xs sm:text-sm">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-bold">{quantidade} unidades</span>
                          </div>
                          <div className="flex justify-between">
        ...
        {!isLoading('carregando') && movimentacoes.length > 0 && (
